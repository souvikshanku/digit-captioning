        ...
        for t in range(self.seq_len):
            for layer in range(self.num_layers):
                h_t[layer] = torch.tanh(
                    x[t] @ self.W_ih[layer].T  # this is wrong actually from layer 2 onwards # noqa: E501
                    + self.b_ih[layer]
...
class LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, vocab_size=29):
        ...
        self.out_fc = nn.Linear(hidden_size, self.vocab_size)
    ...
    def forward(self, inp, img_emb):
        ...
        for t in range(seq_length):
            ...
            gates = self.xh(x_t) + self.hh(h_t_minus_1) + (t == 0) * img_emb
